...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.w_a = nn.Linear(self.num_heads * self.head_size, self.num_heads * self.head_size)
    ...
    def get_attention_weights(self, queries, keys):
        ...
        # print(queries.shape)
        # print(keys.shape)
        mask = (torch.triu(torch.ones(self.sequence_length, self.sequence_length)) == 1).transpose(0, 1)
        # mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        mask = mask.repeat(self.num_heads, 1, 1).cuda()
        # print(mask.shape)
        attention_weights = F.softmax((queries.matmul(keys.transpose(-2, -1)) / math.sqrt(self.head_size)).masked_fill(mask == 0, float('-inf')), dim=-1)
        # print(attention_weights.sum(dim=-2))
        # print(attention_weights[0][0])
        # print('attention_shape {}'.format(attention_weights.shape))
    ...
    def apply_attention(self, queries, keys, values):
        ...
        # print(weights.shape)
        attention_value = weights.matmul(values)
        # print(attention_value.shape)
        out = self.merge_heads(attention_value)
        # print(out.shape)
        return out
    ...
    def split_heads(self, tensor):
        ...
        # print('-------- Here ---------')
        # print(tensor.shape)
        tensor = tensor.reshape([tensor.shape[0], tensor.shape[1], self.num_heads, int(tensor.shape[2]/self.num_heads)])
        # print(tensor.shape)
    ...
    def forward(self, hidden_states):
        ...
        # TODO: Write your code here
        # ==========================
        # qq = self.split_heads(hidden_states)
        # print(qq.shape)
        # Q = self.w_q(qq)
        # print(Q.shape)
        # K = self.w_k(self.split_heads(hidden_states))
        # print(K.shape)
        # V = self.w_v(self.split_heads(hidden_states))
        # print(V.shape)
        # print('---------')
        ...
        k = self.w_k(hidden_states)  # .view(batch_size, -1, self.num_heads, self.head_size)
        # print(k.shape)
        q = self.w_q(hidden_states)  # .view(batch_size, -1, self.num_heads, self.head_size)
        v = self.w_v(hidden_states)  # .view(batch_size, -1, self.num_heads, self.head_size)
        ...
        # print(split_v.shape)
        attn = self.apply_attention(split_q, split_k, split_v)
        # print("attn.shape ", attn.shape)
        output = self.w_a(attn)
        return output
        # return torch.rand([hidden_states.shape[0], self.sequence_length, self.num_heads * self.head_size])
...
class MiniGPT1(nn.Module):
    ...
    def get_embeddings(self, inputs):
        ...
        position = torch.arange(0, self.sequence_length).repeat([batch_size, 1]).cuda()
        # print('----here---')
